...
class AudioEngine {
  ...
  playFootstep() {
    ...
    this.playTone(60 + visualRandom() * 20, 'sine', 0.05, 0.02);
  }
  ...
  playTypewriter() {
    this.init();
    this.playTone(600 + visualRandom() * 200, 'sine', 0.02, 0.02);
  }
  ...
  startAmbient(type: AmbientScene) {
    ...
    this.ambientInterval = window.setInterval(
      () => {
        ...
        if (type === 'proberaum') {
          // Muffled thumping
          this.playTone(40 + visualRandom() * 10, 'sine', 0.5, 0.02);
        } else if (type === 'tourbus') {
          // Engine hum
          this.playTone(50, 'sine', 1.0, 0.01);
        } else if (type === 'backstage') {
          // Muffled crowd chatter
          this.playTone(200 + visualRandom() * 50, 'sine', 0.1, 0.005);
        } else if (type === 'void_station') {
          // Cosmic glitches
          this.playTone(800 + visualRandom() * 2000, 'sawtooth', 0.05, 0.005);
          this.playTone(20 + visualRandom() * 30, 'square', 0.2, 0.05);
          ...
          try {
            ...
            filter.frequency.value = 1000 + visualRandom() * 2000;
            filter.Q.value = 10;
          ...
          if (visualRandom() > 0.9) this.playTone(200 + visualRandom() * 100, 'sine', 2.0, 0.005);
        }
      },
