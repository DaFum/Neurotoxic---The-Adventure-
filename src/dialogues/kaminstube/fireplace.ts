    ...
    options: [
      {
        ...
        action: () => {
          const store = game();
          store.setDialogue('Du verbindest dich mit der uralten Asche. Der Kamin flüstert von Salzgitter: "Dort wird die Grenze zwischen Musik und Realität brechen. Nur eine vereinte Band kann den Riss schließen."');
      ...
      },
      {
        ...
        action: () => {
          const store = game();
          store.setDialogue('Du schreist in die Flammen. Das Feuer lodert rot auf und faucht: "Salzgitter wird brennen! Nur Einigkeit rettet euch vor der Leere!"');
          store.setFlag('forgotten_lore', true);
          store.setFlag('kaminFeuerPact', true);
          store.discoverLore('kamin_prophecy');
          store.completeQuest('forgotten_lore');
      ...
      },
      {
        ...
        action: () => {
          const store = game();
          store.setDialogue('Du decodierst die Frequenzen des Knisterns. Eine Nachricht aus der Vergangenheit: "In Salzgitter wird die Grenze brechen. Vereint die Band."');
          store.setFlag('forgotten_lore', true);
          store.setFlag('kaminFeuerPact', true);
          store.discoverLore('kamin_prophecy');
          store.completeQuest('forgotten_lore');
      ...
      },
      {
        ...
        action: () => {
          const store = game();
          store.setDialogue('Du verstehst das Flüstern! Es erzählt von einem versteckten Archiv unter der Bühne, das die wahren Ursprünge des Industrial Metal enthält. Du hast die Lore entschlüsselt.');
          store.setFlag('forgotten_lore', true);
          store.setFlag('kaminFeuerPact', true);
          store.discoverLore('kamin_prophecy');
          store.completeQuest('forgotten_lore');
