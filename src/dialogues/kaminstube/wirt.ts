...
function buildLegacy1982Dialogue(): Dialogue {
  const store = game();
...
function buildHighMoodDialogue(): Dialogue {
  return {
...
function buildNeutralDialogue(): Dialogue {
  return {
...
export function buildKaminstubeWirtDialogue(): Dialogue {
  ...
  if (store.bandMood > 80) {
    return buildHighMoodDialogue();
  }

  if (store.bandMood < 30) {
    return buildLowMoodDialogue();
  }

  return buildNeutralDialogue();
}
