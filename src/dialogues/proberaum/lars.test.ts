...
describe('buildProberaumLarsDialogue', () => {
  ...
  it('prompts for Mop when missing', () => {
    ...
    expect(getDialogueText(dialogue)).toContain('Ich hab hier irgendwo einen Wischmopp gesehen');
  ...
  it('tells player to wipe puddle when Mop is found but water is not cleaned', () => {
    ...
    const dialogue = buildProberaumLarsDialogue();
    expect(getDialogueText(dialogue)).toContain('Du hast den Mopp! Wisch die Pfütze in der Mitte auf!');
  });
  ...
  it('returns pessimistic dialogue if bandMood is low', () => {
    ...
    const dialogue = buildProberaumLarsDialogue();
    expect(getDialogueText(dialogue)).toContain('Ich pack meine Sticks ein. Dieser Gig wird ein Desaster.');
  });
  ...
  it('returns ready dialogue with trait options when water is cleaned and mood is ok', () => {
    ...
    const dialogue = buildProberaumLarsDialogue();
    expect(getDialogueText(dialogue)).toContain('Geiler Beat, oder?');
  ...
  it('hides secrets if already discovered', () => {
    ...
    const dialogue = buildProberaumLarsDialogue();
    expect(getDialogueText(dialogue)).toContain('Die Hi-Hat ist perfekt. Ich bin bereit.');
    expect(getOptionTexts(dialogue)).toHaveLength(0);
  });
  ...
  it('handles beer dialogue when Lars has not received beer yet', () => {
    ...
    expect(getDialogueText(dialogue)).toContain('Ist das... ein kühles Blondes? Gib her');
    const options = getOptionTexts(dialogue);
    expect(options).toHaveLength(3);
  ...
  it('handles philosophy dialogue without pact when flags are partially set', () => {
    ...
    const dialogue = buildProberaumLarsDialogue();
    expect(getDialogueText(dialogue)).toContain('Du kennst jetzt meine Philosophie. Der Beat ist alles.');
    const options = getOptionTexts(dialogue);
  ...
  it('handles final pact state with beer', () => {
    ...
    const dialogue = buildProberaumLarsDialogue();
    expect(getDialogueText(dialogue)).toContain('Der Pakt steht. Wir sind das Skelett der Welt. Und... ist das ein kühles Blondes?');
    const options = getOptionTexts(dialogue);
