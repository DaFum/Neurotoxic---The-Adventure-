...
export function buildProberaumDrumMachineDialogue(): Dialogue {
  const store = game();
  const { flags, hasItem } = store;
  const drumMachineQuestStatus = getCachedQuest('drum_machine')?.status; // Bolt: O(1) cache lookup
...
export function buildProberaumMonitorDialogue(): Dialogue {
  const store = game();
  const { flags, hasItem } = store;
  const feedbackMonitorQuestStatus = getCachedQuest('feedback_monitor')?.status; // Bolt: O(1) cache lookup
