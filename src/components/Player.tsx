...
export function Player({ bounds = { x: [-10, 10], z: [-5, 5] } }: PlayerProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = useStore.subscribe((state) => {
      ...
      try {
        ...
        const dx =
          Math.abs(cur.x - newPos[0]) + Math.abs(cur.y - newPos[1]) + Math.abs(cur.z - newPos[2]);
  ...
  useFrame((state, delta) => {
    ...
    bodyRef.current.setLinvel(
      { x: velocity.x, y: bodyRef.current.linvel().y, z: velocity.z },
      true,
    );
    ...
    if (dx * dx + dy * dy + dz * dz > 0.05) {
      ...
      !moving &&
      (lastSentPosRef.x !== clampedX || lastSentPosRef.y !== pos.y || lastSentPosRef.z !== clampedZ)
    ) {
    ...
    state.camera.position.x = THREE.MathUtils.lerp(
      ...
      clampedX + shakeOffset.x,
      0.1,
    );
    state.camera.position.z = THREE.MathUtils.lerp(
      state.camera.position.z,
      clampedZ + 10 + shakeOffset.z,
      0.1,
    );
    ...
    <RigidBody ref={bodyRef} position={[0, 1, 0]} lockRotations colliders={false}>
      ...
      <group ref={modelRef}>
        ...
        <sprite ref={spriteRef} scale={[2.3, 2.3, 1]} renderOrder={20}>
          <spriteMaterial
            map={playerTextureRef.current ?? undefined}
            transparent
            depthWrite={false}
            depthTest={false}
          />
