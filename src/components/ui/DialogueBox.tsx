...
export function DialogueBox({ dialogue, setDialogue, questDictionary }: DialogueBoxProps) {
  useStore(
    useShallow((state) => ({
      flags: state.flags,
      trait: state.trait,
      ...
      <AnimatePresence>
        {dialogue && (
          <div
            ...
            tabIndex={-1}
            className="absolute bottom-12 left-1/2 -translate-x-1/2 w-full max-w-2xl pointer-events-auto focus:outline-none"
            role="dialog"
            aria-modal="true"
                          ...
                          return (
                            ...
                            >
                              ...
                              {(skillReq ||
                                ...
                                >
                                  {skillReq &&
                                    `[ REQ: ${skillReq.name.toUpperCase()} ${skillReq.level} ] `}
                                  ...
                                  {questDeps &&
                                    questDeps.map((dep, depIdx) => {
                                      if (typeof dep === 'string') {
                                        ...
                                        return (
                                          <span key={`dep-${depIdx}`}>
                                            [ REQ: QUEST {dep.status.toUpperCase()}: {questTitle}{' '}
                                            ]{' '}
                                          </span>
                                        );
                                      }
                    ...
                    {(!dialogue?.options || dialogue.options.length === 0) && (
                      ...
                      >
                        {displayedText.length < (dialogue?.text.length || 0)
                          ? 'SKIP_DATA'
                          : 'ACKNOWLEDGE'}
                        <span className="opacity-50 ml-2 font-mono">[SPACE]</span>
                      </button>
                    )}
                  </div>
                </div>
