      ...
      return (
        <group>
          ...
          {PROBERAUM_CABLE_X_POSITIONS.map((x, idx) => (
            <mesh
              key={`proberaum-cable-${x}`}
              position={[x, 0.12, 6.6]}
              rotation={[-Math.PI / 2, idx * 0.2, 0]}
            >
              <torusGeometry args={[0.28, 0.04, 10, 20]} />
              <meshStandardMaterial
                color={idx % 2 === 0 ? '#242d3b' : '#35243b'}
                metalness={0.58}
                roughness={0.34}
              />
            </mesh>
          ...
          {TOURBUS_OVERHEAD_X_POSITIONS.map((x, idx) => (
            <group key={`tourbus-overhead-${x}`} position={[x, 3.55, -0.4]}>
              ...
              <mesh position={[0, -0.1, 0]}>
                ...
                <meshStandardMaterial
                  color={idx % 2 === 0 ? '#66dcff' : '#ff9968'}
                  emissive={idx % 2 === 0 ? '#66dcff' : '#ff9968'}
      ...
      return (
        <group>
          ...
          {BACKSTAGE_LAMP_X_POSITIONS.map((x, idx) => (
            <group key={`backstage-lamp-${x}`} position={[x, 6.95, 1.2]}>
              ...
              <mesh position={[0, -0.52, 0]}>
                ...
                <meshStandardMaterial
                  color={idx % 2 === 0 ? '#8eff70' : '#79ddff'}
                  emissive={idx % 2 === 0 ? '#8eff70' : '#79ddff'}
          ...
          {BACKSTAGE_RUNWAY_X_POSITIONS.map((x, idx) => (
            <mesh
              key={`backstage-runway-led-${x}`}
              position={[x, 0.03, 1.7]}
              rotation={[-Math.PI / 2, 0, 0]}
            >
              <planeGeometry args={[2.5, 0.14]} />
              <meshStandardMaterial
                color={idx % 2 === 0 ? '#d6ff4a' : '#ff6aa8'}
                emissive={idx % 2 === 0 ? '#d6ff4a' : '#ff6aa8'}
      ...
      return (
        <group>
          {VOID_RING_RADII.map((r, idx) => (
            ...
            >
              <mesh
                position={[0, 2.2 + idx * 1.1, -1.8 - idx * 1.2]}
                rotation={[Math.PI / 2, 0, idx * 0.35]}
              >
                <torusGeometry args={[r, 0.14, 16, 88]} />
                <meshStandardMaterial
                  color={idx % 2 === 0 ? '#66f3ff' : '#ad78ff'}
                  emissive={idx % 2 === 0 ? '#2aa8b7' : '#6b42ba'}
          ...
          {VOID_PYLON_POSITIONS.map((pos, idx) => (
            <group key={`void-pylon-${idx}`} position={pos}>
              ...
              <mesh position={[0, 1.24, 0]}>
                ...
                <meshStandardMaterial
                  color={idx % 2 === 0 ? '#74f5ff' : '#ff7de0'}
                  emissive={idx % 2 === 0 ? '#39aab3' : '#a94192'}
          ...
          {VOID_STRIP_X_POSITIONS.map((x, idx) => (
            <mesh key={`void-strip-${x}`} position={[x, 0.05, -9]} rotation={[-Math.PI / 2, 0, 0]}>
              ...
              <meshStandardMaterial
                color={idx % 2 === 0 ? '#62ffd8' : '#ff76e0'}
                emissive={idx % 2 === 0 ? '#62ffd8' : '#ff76e0'}
        ...
        <group>
            ...
            <group key={`salzgitter-tower-${idx}`} position={pos}>
              ...
              {SALZGITTER_TOWER_STRIP_Y_OFFSETS.map((y, row) => (
                <mesh key={`salzgitter-tower-strip-${idx}-${row}`} position={[0, y, 0.56]}>
                  ...
                  <meshStandardMaterial
                    color={row % 2 === 0 ? '#6ef5ff' : '#ff73d7'}
                    emissive={row % 2 === 0 ? '#6ef5ff' : '#ff73d7'}
          ...
          {SALZGITTER_GANTRY_X_POSITIONS.map((x, idx) => (
            <mesh key={`salzgitter-gantry-lamp-${x}`} position={[x, 7, -8.1]}>
              ...
              <meshStandardMaterial
                color={idx % 2 === 0 ? '#2dffe7' : '#ff5ecb'}
                emissive={idx % 2 === 0 ? '#2dffe7' : '#ff5ecb'}
          ...
          {SALZGITTER_FRONT_STRIP_X_POSITIONS.map((x, idx) => (
            <mesh
              key={`salzgitter-front-strip-${x}`}
              position={[x, 0.03, 6.2]}
              rotation={[-Math.PI / 2, 0, 0]}
            >
              <planeGeometry args={[2.8, 0.12]} />
              <meshStandardMaterial
                color={idx % 2 === 0 ? '#5aa0ff' : '#ff73cb'}
                emissive={idx % 2 === 0 ? '#5aa0ff' : '#ff73cb'}
