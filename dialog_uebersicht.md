...
_Update 02.04.2026 (Performance/Anti-Farm Logic): Die automatische Stacktrace-Deduplizierung für `increaseBandMood` wurde entfernt. Alle einmaligen positiven BandMood-Belohnungen erfordern nun einen expliziten, stabilen `sourceId` Parameter (z.B. `increaseBandMood(10, 'id_1234abcd')`), andernfalls kann der Boost unendlich oft gefarmt werden._
...
_Update 02.04.2026 (Quest API Error Handling): Die Quest-API (`completeQuest`, `failQuest`, `completeQuestWithFlag`) wirft nun einen Error, wenn versucht wird, eine nicht registrierte Quest ohne Text abzuschließen oder fehlschlagen zu lassen. Der `dialogueEngine` fängt diese Fehler ab, um Abstürze zu vermeiden._
...
- **Ersatzröhre (Item):**
  - _Interaktion:_ Aufheben (Erhalt: Röhre). Bei erreichtem Pickup-Limit erscheint ein expliziter Hinweistext statt stiller Erfolgsannahme. _Hinweis: Der Pickup wird versteckt, sobald die `amp` Quest abgeschlossen ist (oder `ampRepaired` gesetzt ist)._
- **Kaputter Amp:**
